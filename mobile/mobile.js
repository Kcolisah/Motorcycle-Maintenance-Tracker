...
function renderBackendNotice(context = "mobile") {
  ...
  return `
    <div class="mobile-data-notice">
      ...
      <span>Backend data is unavailable, so ${escapeHtml(context)} is using safe demo data.</span>
...
function renderHome() {
  ...
  return `
    <section class="screen home-screen">
      ...
      ${renderBackendNotice("the home preview")}
  ...
  return `
    ...
    <section class="bottom-sheet bike-sheet" data-floating-panel role="dialog" aria-label="Bike actions">
      ...
      <div class="sheet-action-group">
        ...
        <button class="sheet-action-btn danger-soft" type="button" data-action="bike-remove-placeholder">Remove placeholder</button>
      </div>
    </section>
  `;
}
...
function renderOverlay() {
  ...
  overlayLayer.classList.toggle("has-sheet", Boolean(state.activeTaskId || state.activeBikeActionId));
}
...
async function handleActionClick(target) {
  ...
  if (action === "bike-remove-placeholder") {
    showToast("Remove bike will be wired after confirmation UI.");
    return true;
  }
